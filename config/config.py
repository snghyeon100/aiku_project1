...
class LOBSTER(Dataset):
    ...
    dates: list = field(default_factory=lambda: ["2025-05-01", "2025-05-10"])
...
class BTC(Dataset):
    ...
    dates: list = field(default_factory=lambda: ["2025-05-11", "2025-05-20"])
...
@dataclass
class BINANCE(Dataset):
    type: DatasetType = DatasetType.BINANCE   # DatasetType에 BINANCE가 없다면 BTC로 둬도 무방
    dates: list = field(default_factory=lambda: ["2025-05-01", "2025-05-10"])
    batch_size: int = 64
    sampling_type: SamplingType = SamplingType.NONE
    sampling_time: str = "100ms"
...
cs.store(group="dataset", name="binance", node=BINANCE)