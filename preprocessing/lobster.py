            ...
            if os.path.isfile(f):
                ...
                if i < split_days[0]:
                    if (i % 2) == 0:
                        ...
                        if i == 1:
                            train_orderbooks = pd.read_csv(f, names=COLUMNS_NAMES["orderbook"],header=None)
                            total_shape += train_orderbooks.shape[0]
                            min_len = min(len(train_messages), len(train_orderbooks))
                            train_messages = train_messages.iloc[:min_len].reset_index(drop=True)
                            train_orderbooks = train_orderbooks.iloc[:min_len].reset_index(drop=True)
                            ...
                            if (len(train_orderbooks) != len(train_messages)):
                                print(len(train_orderbooks), len(train_messages))
