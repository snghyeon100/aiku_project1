...
def train(config: Config, trainer: L.Trainer, run=None):
    ...
    if dataset_type == "FI_2010":
        ...
        train_input, train_labels = btc_load(cst.DATA_DIR + "/BINANCE/train.npy", cst.LEN_SMOOTH, horizon, seq_size)
        val_input, val_labels = btc_load(cst.DATA_DIR + "/BINANCE/val.npy", cst.LEN_SMOOTH, horizon, seq_size)  
        test_input, test_labels = btc_load(cst.DATA_DIR + "/BINANCE/test.npy", cst.LEN_SMOOTH, horizon, seq_size)
        train_set = Dataset(train_input, train_labels, seq_size)
        val_set = Dataset(val_input, val_labels, seq_size)
        test_set = Dataset(test_input, test_labels, seq_size)
        if config.experiment.is_debug:
        ...
        print("▶ 라벨 값 분포 확인")
        print("train_labels:", torch.unique(train_labels, return_counts=True))
        print("val_labels:", torch.unique(val_labels, return_counts=True))
        print("test_labels:", torch.unique(test_labels, return_counts=True))
       
        n_classes = 3
        invalid = (train_labels < 0) | (train_labels >= n_classes)
        if invalid.any():
            print(f"⚠️ train_labels에서 유효하지 않은 값 {invalid.sum().item()}개 발견 → 클래스 1로 수정")
            train_labels = torch.where(invalid, torch.tensor(1, dtype=train_labels.dtype), train_labels)
